...
*** Test Cases ***
TC_SIMRPL_001 SIM Replacement Blank Order Creation And Preparation
    ...
    ...
    ...                Suite variables set for TC_SIMRPL_003 (same session) or override via env/seed for standalone:
    ...
    [Tags]    sim-replacement    regression    smoke    TC_SIMRPL_001
    ...
    Log    TC_SIMRPL_001 complete — MSISDN=${SIMR_MSISDN} OLD_ICCID=${SIMR_OLD_ICCID} NEW_ICCID=${SIMR_NEW_ICCID} ORDER=${SIMR_SIM_ORDER_ID}    console=yes

TC_SIMRPL_003 Perform SIM Replacement And Verify
    ...
    ...
    ...                Designed to run after TC_SIMRPL_001 in the same suite session (picks up suite variables
    ...
    ...
    [Tags]    sim-replacement    regression    smoke    TC_SIMRPL_003
    Should Not Be Empty    ${SIMR_MSISDN}
    ...    msg=SIMR_MSISDN required — run TC_SIMRPL_001 first (same suite session) or set env STC_SIMR_MSISDN / seed simr_msisdn.
    Should Not Be Empty    ${SIMR_OLD_ICCID}
    ...    msg=SIMR_OLD_ICCID required — run TC_SIMRPL_001 first (same suite session) or set env STC_SIMR_OLD_ICCID / seed simr_old_iccid.
...
TC_SIMRPL_004 Verify Notes After Replacement
    [Documentation]    Requires SIMR_MSISDN and completed replacement (e.g. after TC_SIMRPL_001).
    [Tags]    regression    TC_SIMRPL_004
    Should Not Be Empty    ${SIMR_MSISDN}
    SR Expand First Row Open Notes And Assert Replacement    ${SIMR_MSISDN}

TC_SIMRPL_005 Verify New SIM On Blank SIM Tab
    [Documentation]    Requires ${SIMR_NEW_ICCID} from TC_SIMRPL_001 (same suite session).
    [Tags]    regression    TC_SIMRPL_005
    Should Not Be Empty    ${SIMR_NEW_ICCID}    msg=Run TC_SIMRPL_001 first or set suite variable SIMR_NEW_ICCID.
    SR Navigate To Blank Sims Page
    SR Search Grid On Current Page    ${SIMR_NEW_ICCID}
    SR Verify Blank Sim Row In Use

TC_SIMRPL_006 Verify Old SIM On Lost SIM Module
    [Documentation]    Requires ${SIMR_OLD_ICCID} / ${SIMR_OLD_IMSI} from TC_SIMRPL_001.
    [Tags]    regression    TC_SIMRPL_006
    Should Not Be Empty    ${SIMR_OLD_ICCID}    msg=Run TC_SIMRPL_001 first.
    SR Navigate To Lost Sims Page
    SR Search Grid On Current Page    ${SIMR_OLD_ICCID}
    SR Verify Lost Sim Row For Old Iccid

TC_SIMRPL_007 Negative No IMSI Options Submit Disabled
    ...
    [Tags]    sim-replacement    negative    TC_SIMRPL_007
    Pass Execution If    '${SIMR_MSISDN}' == '${EMPTY}'
    ...    No MSISDN: run TC_SIMRPL_001 first in this suite or set STC_SIMR_MSISDN / seed simr_msisdn.
